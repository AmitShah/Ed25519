...
import { BaseValidationModule, ECDSAOwnershipValidationModuleConfig, ECDSAOwnershipValidationModuleConfigConstructorProps } from "@biconomy/modules";


// Could be renamed with suffix API
export class EDDSAOwnershipValidationModule extends BaseValidationModule {
  ...
  public static async create(moduleConfig: ECDSAOwnershipValidationModuleConfig): Promise<EDDSAOwnershipValidationModule> {
    ...
    const instance = new EDDSAOwnershipValidationModule(configForConstructor);
    instance.moduleAddress = moduleConfig.moduleAddress as `0x${string}`;
